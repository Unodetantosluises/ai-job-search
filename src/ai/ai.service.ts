...
`;
  ...
  async draftLatex(
    ...
    try {
      ...
      if (downskilling) {
        try {
          const downskillingPath = path.join(
            process.cwd(),
            'docs_prompts',
            'skills',
            'job-application-assistant',
            '08-cv-downskilling.md',
          );
...
`;
  ...
  /**
   * Genera el contenido de 02-behavioral-profile.md a partir de las respuestas
   * del usuario recolectadas via InquirerService durante el setup interactivo.
   */
  async generateBehavioralProfile(answers: Record<string, string>): Promise<string> {
    if (!this.genAI) {
      throw new Error('El cliente de Gemini no está inicializado. Verifica tu GEMINI_API_KEY.');
    }
...
${answersText}`;
  ...
  /**
   * Genera el bloque de metas de carrera para insertar en 04-job-evaluation.md
   * a partir de las respuestas del usuario sobre objetivos y preferencias de tareas.
   */
  async generateCareerGoals(answers: Record<string, string>): Promise<string> {
    if (!this.genAI) {
      throw new Error('El cliente de Gemini no está inicializado. Verifica tu GEMINI_API_KEY.');
    }
...
Sé honesto y específico. No generes metas genéricas como "crecer profesionalmente". Usa el lenguaje del candidato.`;
    ...
    // Sanitizar bloques de código si el modelo los añade
